/**
 * The selectable catalogues the setup screen is built from (PRD §6–§10).
 *
 * Everything here is data, not UI: the server uses the same lists to validate
 * incoming project configurations and to give the production team concrete
 * vocabulary (an erhu is bowed and held vertically on the thigh; a taiko is
 * struck standing) so prompts describe a player who is actually playing.
 */

export interface InstrumentCategory {
  id: string;
  /** Spanish label, matching the PRD's category list. */
  label: string;
}

export interface Instrument {
  id: string;
  /** Common name shown in the picker. */
  name: string;
  categoryId: string;
  /** Extra search terms: local names, alternate spellings, English names. */
  aliases: string[];
  /** How the instrument is played — feeds the Art Director's continuity notes. */
  technique: string;
  /** How the performer physically holds it — feeds anatomy-safe prompts. */
  posture: string;
  /** Cultural / stylistic origin, used for scene and music briefs. */
  origin: string;
}
...
const I = (
  ...
  origin: string,
  aliases: string[] = [],
): Instrument => ({ id, name, categoryId, technique, posture, origin, aliases });
...
/** Case/accent-insensitive instrument search for the picker (PRD §6.1). */
export function searchInstruments(query: string, limit = 30): Instrument[] {
...
export interface FormationOption {
  id: string;
  label: string;
  /** Number of visible performers the Art Director should stage. */
...
export const FORMATIONS_BY_ID = new Map(FORMATIONS.map((f) => [f.id, f]));

// --- Intérprete (PRD §8) -----------------------------------------------------

export interface PerformerGenderOption {
  id: string;
  label: string;
}

export const PERFORMER_GENDERS: PerformerGenderOption[] = [
  { id: 'female', label: 'Femenino' },
  { id: 'male', label: 'Masculino' },
  { id: 'mixed', label: 'Mixto' },
];

export interface PerformerTypeOption {
  id: string;
  label: string;
  genderIds: string[];
  /** How the Art Director should describe the figure. */
  descriptor: string;
  plural: boolean;
}

export const PERFORMER_TYPES: PerformerTypeOption[] = [
...
export const PERFORMER_TYPES_BY_ID = new Map(PERFORMER_TYPES.map((p) => [p.id, p]));
...
export interface ScenarioOption {
  id: string;
  label: string;
  /** Visual anchors the Art Director must keep consistent between shots. */
  elements: string[];
  /** Ambient sound palette (PRD §31). */
  ambience: string[];
  /** Whether the scene is outdoors — drives lighting and wind behaviour. */
  outdoor: boolean;
  /** Default reverberation character for the audio mix. */
  acoustics: 'dry' | 'natural' | 'reverberant' | 'cavernous';
}
...
export interface VisualStyleOption {
  id: string;
  label: string;
  /** Rendering language handed to the image model. */
  treatment: string;
  /** Lens / grain language handed to the video model. */
  photography: string;
  /** Base colour direction. */
  palette: string[];
}
...
export const VISUAL_STYLES_BY_ID = new Map(VISUAL_STYLES.map((s) => [s.id, s]));
